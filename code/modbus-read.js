...
module.exports = function (RED) {
  ...
  var internalDebugLog = require('debug')('contribModbus:read');
  ...
  function ModbusRead(config) {
    ...
    function sendMessage(values, response, msg) {
      ...
      if (node.useIOFile && node.ioFile.lastUpdatedAt) {
        ...
        var allValueNames = mbIOCore.nameValuesFromIOFile(node, msg, values, response, node.adr);
        var valueNames = mbIOCore.filterValueNames(node, allValueNames, mbCore.functionCodeModbusRead(node.dataType), node.adr, node.quantity);
        var origMsg = {
          topic: topic,
    ...
    function setNodeStatusWithTimeTo(statusValue) {
      ...
      var statusOptions = mbBasics.setNodeStatusProperties(statusValue, node.showStatusActivities);
      var statusText = node.statusText;

      if (statusValue.search('active') !== -1 || statusValue === 'polling') {
        var newStatusText = statusOptions.status + getTimeInfo();
        ...
        var _newStatusText = statusOptions.status;

        if (_newStatusText !== statusText) {
          node.status({
            fill: statusOptions.fill,
            shape: statusOptions.shape,
            text: _newStatusText
  ...
  RED.httpAdmin.post('/modbus/read/inject/:id', RED.auth.needsPermission('modbus.inject.write'), function (req, res) {
    ...
    if (node) {
      try {
        ...
        res.sendStatus(500);
        node.error(RED._('modbusinject.failed', {
          error: err.toString()
        }));
      }
    } else {
