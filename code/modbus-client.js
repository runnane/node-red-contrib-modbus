...
module.exports = function (RED) {
  ...
  var coreModbusClient = require('./core/modbus-client-core');

  var coreModbusQueue = require('./core/modbus-queue-core');

  var internalDebugLog = require('debug')('contribModbus:config:client');

  function ModbusClientNode(config) {
    RED.nodes.createNode(this, config); // create an empty modbus client

    var ModbusRTU = require('modbus-serial');

    var unlimitedListeners = 0;
    var minCommandDelayMilliseconds = 1;
    var defaultUnitId = 1;
    var defaultTcpUnitId = 0;
    var serialConnectionDelayTimeMS = 500;
    var timeoutTimeMS = 1000;
    var reconnectTimeMS = 2000;
    var logHintText = ' Get More About It By Logging';
    var serialAsciiResponseStartDelimiter = '0x3A';
    ...
    node.setUnitIdFromPayload = function (msg) {
      var unitId = coreModbusClient.getActualUnitId(node, msg);
    ...
    node.stateService.subscribe(function (state) {
      node.actualServiceStateBefore = node.actualServiceState;
      ...
      if (state.matches('init')) {
        ...
        try {
          ...
          node.error(err, {
            payload: 'client connection error ' + logHintText
          });
        }
    ...
    node.connectClient = function () {
      ...
      if (node.clienttype === 'tcp') {
        if (!coreModbusClient.checkUnitId(node.unit_id, node.clienttype)) {
          node.error(new Error('wrong unit-id (0..255)'), {
            payload: node.unit_id
          });
        ...
        switch (node.tcpType) {
          case 'C701':
            ...
            node.client.connectC701(node.tcpHost, {
              port: node.tcpPort,
              autoOpen: true
            }).then(node.setTCPConnectionOptions).then(node.setTCPConnected)["catch"](node.modbusTcpErrorHandling);
            break;
          ...
          case 'TELNET':
            ...
            node.client.connectTelnet(node.tcpHost, {
              port: node.tcpPort,
              autoOpen: true
            }).then(node.setTCPConnectionOptions)["catch"](node.modbusTcpErrorHandling);
            break;
        ...
        if (!coreModbusClient.checkUnitId(node.unit_id, node.clienttype)) {
          node.error(new Error('wrong unit-id serial (0..247)'), {
            payload: node.unit_id
          });
          node.stateService.send('FAILURE');
          return;
        ...
        if (!node.serialPort) {
          node.error(new Error('wrong serial port'), {
            payload: node.serialPort
          });
          node.stateService.send('FAILURE');
          return;
        }
        ...
        switch (node.serialType) {
          case 'ASCII':
            verboseLog('ASCII port serial'); // Make sure is parsed when string, otherwise just assign.
            ...
            node.client.connectAsciiSerial(node.serialPort, serialPortOptions).then(node.setSerialConnectionOptions)["catch"](node.modbusSerialErrorHandling);
            break;

          case 'RTU':
            verboseLog('RTU port serial');
            node.client.connectRTU(node.serialPort, serialPortOptions).then(node.setSerialConnectionOptions)["catch"](node.modbusSerialErrorHandling);
            break;

          default:
            verboseLog('RTU buffered port serial');
            node.client.connectRTUBuffered(node.serialPort, serialPortOptions).then(node.setSerialConnectionOptions)["catch"](node.modbusSerialErrorHandling);
            break;
        }
      }
    };
    ...
    node.modbusTcpErrorHandling = function (err) {
      ...
      if (err.errno && coreModbusClient.networkErrors.includes(err.errno) || err.code && coreModbusClient.networkErrors.includes(err.code)) {
        node.stateService.send('BREAK');
      }
    };
    ...
    node.on('close', function (done) {
      var nodeIdentifierName = node.name || node.id;
