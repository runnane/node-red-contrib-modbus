...
module.exports = function (RED) {
  ...
  var internalDebugLog = require('debug')('contribModbus:flex:server');
  ...
  function ModbusFlexServer(config) {
    ...
    mbBasics.setNodeStatusTo('initialized', node); //     1...10000*  address - 1      Coils (outputs)    0   Read/Write
    ...
    function buildMessage(msg) {
      return [{
        type: 'holding',
        message: msg,
        payload: node.registers.slice(node.splitAddress * coreServer.bufferFactor)
      }, {
        type: 'coils',
        message: msg,
        payload: node.coils.slice(0, node.splitAddress * coreServer.bufferFactor)
      }, {
        type: 'input',
        message: msg,
        payload: node.registers.slice(0, node.splitAddress * coreServer.bufferFactor)
      }, {
        type: 'discrete',
        message: msg,
        payload: node.coils.slice(node.splitAddress * coreServer.bufferFactor)
      }, {
        payload: 'request',
        type: 'message',
        message: msg
