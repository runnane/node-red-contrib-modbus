...
de.biancoroyal.modbus.basics.setNodeStatusProperties = function (statusValue, showActivities) {
  ...
  var statusText = statusValue.value || statusValue;
...
de.biancoroyal.modbus.basics.setNodeStatusByResponseTo = function (statusValue, response, node) {
  ...
  node.status({
    fill: fillValue,
    shape: shapeValue,
    text: this.util.inspect(response, false, null)
  });
};
...
de.biancoroyal.modbus.basics.setNodeStatusTo = function (statusValue, node) {
  if (node.showStatusActivities) {
    if (statusValue !== node.statusText) {
      var statusOptions = this.setNodeStatusProperties(statusValue, node.showStatusActivities);
...
de.biancoroyal.modbus.basics.initModbusClientEvents = function (node, modbusClient) {
  ...
  if (node.showStatusActivities) {
    modbusClient.on('mbinit', function () {
      _this.onModbusInit(node);
    });
    modbusClient.on('mbqueue', function () {
      _this.onModbusQueue(node);
    });
    modbusClient.on('mbconnected', function () {
      _this.onModbusConnect(node);
    });
    modbusClient.on('mbbroken', function () {
      _this.onModbusBroken(node, modbusClient);
    });
    modbusClient.on('mbactive', function () {
      _this.onModbusActive(node);
    });
    modbusClient.on('mberror', function (failureMsg) {
      _this.onModbusError(node, failureMsg);
    });
    modbusClient.on('mbclosed', function () {
      _this.onModbusClose(node);
    });
  } else {
    this.setNodeDefaultStatus(node);
  }
