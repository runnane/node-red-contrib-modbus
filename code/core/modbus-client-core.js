...
de.biancoroyal.modbus.core.client.networkErrors = ['ESOCKETTIMEDOUT', 'ETIMEDOUT', 'ECONNRESET', 'ENETRESET', 'ECONNABORTED', 'ECONNREFUSED', 'ENETUNREACH', 'ENOTCONN', 'ESHUTDOWN', 'EHOSTDOWN', 'ENETDOWN', 'EWOULDBLOCK', 'EAGAIN', 'EHOSTUNREACH'];

de.biancoroyal.modbus.core.client.createStateMachineService = function () {
  this.stateLogEnabled = false; // failure is a general gate point in states to jump between states
...
de.biancoroyal.modbus.core.client.readModbusByFunctionCodeOne = function (node, msg, cb, cberr) {
  var coreClient = de.biancoroyal.modbus.core.client;
...
de.biancoroyal.modbus.core.client.readModbusByFunctionCode = function (node, msg, cb, cberr) {
  var coreClient = de.biancoroyal.modbus.core.client;
  var nodeLog = de.biancoroyal.modbus.core.client.getLogFunction(node);
...
de.biancoroyal.modbus.core.client.writeModbusByFunctionCodeFifteen = function (node, msg, cb, cberr) {
  ...
  if (parseInt(msg.payload.value.length) !== parseInt(msg.payload.quantity)) {
    coreClient.activateSendingOnFailure(node, cberr, new Error('Quantity should be less or equal to coil payload array length: ' + msg.payload.value.length + ' Addr: ' + msg.payload.address + ' Q: ' + msg.payload.quantity), msg);
  } else {
...
de.biancoroyal.modbus.core.client.writeModbusByFunctionCodeSixteen = function (node, msg, cb, cberr) {
  ...
  if (parseInt(msg.payload.value.length) !== parseInt(msg.payload.quantity)) {
    coreClient.activateSendingOnFailure(node, cberr, new Error('Quantity should be less or equal to register payload array length: ' + msg.payload.value.length + ' Addr: ' + msg.payload.address + ' Q: ' + msg.payload.quantity), msg);
  } else {
...
de.biancoroyal.modbus.core.client.setNewSerialNodeSettings = function (node, msg) {
  ...
  node.serialType = msg.payload.serialType || node.serialType; // Make sure is parsed when string, otherwise just assign.
